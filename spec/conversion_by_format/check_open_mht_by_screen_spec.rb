...
palladium = PalladiumHelper.new(DocumentServerHelper.get_version, 'Convert MHT')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.files_from_folder('mht')

describe 'Convert mht files by convert service' do
  before do
    @metadata = nil
  end

  (files - result_sets.map { |result_set| "mht/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      expect(@metadata[:size]).to be > StaticData::MIN_MHT_IMAGE_SIZE
    end
  end
