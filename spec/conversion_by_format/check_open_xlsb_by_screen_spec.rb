...
palladium = PalladiumHelper.new(DocumentServerHelper.get_version, 'Convert XLSB')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.files_from_folder('xlsb')
describe 'Convert docx files by convert service' do
  ...
  (files - result_sets.map { |result_set| "xlsb/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      expect(@metadata[:size]).to be > StaticData::MIN_XLSB_IMAGE_SIZE
    end
  end
