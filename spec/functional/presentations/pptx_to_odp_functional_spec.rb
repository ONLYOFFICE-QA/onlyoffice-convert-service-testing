...
palladium = PalladiumHelper.new DocumentServerHelper.get_version, 'Pptx to Odp'
result_sets = palladium.get_result_sets StaticData::POSITIVE_STATUSES
files = s3.files_from_folder('pptx')
describe 'Convert pptx to odp by convert service' do
  before do
    @metadata = nil
  end
  ...
  (files - result_sets.map { |result_set| "pptx/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      @metadata = converter.perform_convert(url: file_uri(file_path), outputtype: 'odp')
