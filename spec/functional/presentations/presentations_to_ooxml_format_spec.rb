...
palladium = PalladiumHelper.new DocumentServerHelper.get_version, 'Presentations to Ooxml'
result_sets = palladium.get_result_sets StaticData::POSITIVE_STATUSES
files = JSON.load_file(File.join(Dir.pwd, 'assets', 'testing_presentations.json'))['presentations_to_ooxml']

describe 'Convert presentations to ooxml by convert service' do
  before do
    @metadata = nil
  ...
  files.each do |s3_file_path|
    ...
    it test_name do
      ...
      expect(data.at('FileResult/FileType').text).to eq('pptx')
