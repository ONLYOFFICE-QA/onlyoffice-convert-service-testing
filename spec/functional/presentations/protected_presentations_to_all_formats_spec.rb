...
files = JSON.load_file(File.join(Dir.pwd, 'assets', 'testing_presentations.json'))['protected_presentations']
...
describe 'Convert protected presentations to all formats by convert service' do
  ...
  files.each do |s3_file_path|
    ...
    out_formats = output_formats.key?(input_format) ? output_formats[input_format] : output_formats['presentations']
