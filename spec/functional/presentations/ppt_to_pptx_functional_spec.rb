...
palladium = PalladiumHelper.new DocumentServerHelper.get_version, 'Ppt to Pptx'
result_sets = palladium.get_result_sets StaticData::POSITIVE_STATUSES
files = s3.files_from_folder('ppt')
describe 'Convert ppt to pptx by convert service' do
  before do
    @metadata = nil
  end

  (files - result_sets.map { |result_set| "ppt/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      next if StaticData::EXCEPTION_FILES['empty_files'].include?(File.basename(file_path))
