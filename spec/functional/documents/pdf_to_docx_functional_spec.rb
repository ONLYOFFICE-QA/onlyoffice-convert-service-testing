...
palladium = PalladiumHelper.new DocumentServerHelper.get_version, 'Pdf to Docx'
result_sets = palladium.get_result_sets StaticData::POSITIVE_STATUSES
files = s3.files_from_folder('pdf')
describe 'Convert pdf to docx by convert service' do
  before do
    @metadata = nil
  end

  (files - result_sets.map { |result_set| "pdf/#{result_set}" }).each do |file_path|
