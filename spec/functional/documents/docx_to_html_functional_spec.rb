...
palladium = PalladiumHelper.new DocumentServerHelper.get_version, 'Docx to Html'
result_sets = palladium.get_result_sets StaticData::POSITIVE_STATUSES
files = s3.files_from_folder('docx')
describe 'Convert docx to html by convert service' do
  before do
    @metadata = nil
  end
  ...
  (files - result_sets.map { |result_set| "docx/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      @metadata = converter.perform_convert(url: file_uri(file_path), outputtype: 'html')
