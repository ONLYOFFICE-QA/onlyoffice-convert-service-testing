...
palladium = PalladiumHelper.new(DocumentServerHelper.get_version, 'Convert XML')
result_sets = palladium.get_result_sets(StaticData::POSITIVE_STATUSES)
files = s3.files_from_folder('xml')
describe 'Convert docx files by convert service' do
  before do
    @metadata = nil
  end

  (files - result_sets.map { |result_set| "xml/#{result_set}" }).each do |file_path|
    it File.basename(file_path) do
      ...
      expect(@metadata[:size]).to be > StaticData::MIN_XML_IMAGE_SIZE
    end
  end
